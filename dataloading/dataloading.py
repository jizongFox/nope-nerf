...
def get_data_fields(cfg, mode="train"):
    ...
    fields = {}
    load_ref_img = (cfg["training"]["pc_weight"] != 0.0) or (
        cfg["training"]["rgb_s_weight"] != 0.0
    )
    ...
    if dataset_name == "any":
        img_field = DataField(
            ...
            use_DPT=use_DPT,
            mode=mode,
            spherify=cfg["dataloading"]["spherify"],
            ...
            load_colmap_poses=cfg["dataloading"]["load_colmap_poses"],
            sample_rate=cfg["dataloading"]["sample_rate"],
        )
    else:
