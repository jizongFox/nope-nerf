...
def _load_data(
    ...
    if crop_size != 0:
        ...
        for f in sorted(os.listdir(os.path.join(basedir, "images"))):
            if f.endswith("JPG") or f.endswith("jpg") or f.endswith("png"):
                ...
                image_cropped = image[
                    crop_size_H : H - crop_size_H, crop_size_W : W - crop_size_W
                ]
    ...
    img0 = [
        os.path.join(basedir, img_folder, f)
        for f in sorted(os.listdir(os.path.join(basedir, img_folder)))
        if f.endswith("JPG") or f.endswith("jpg") or f.endswith("png")
    ][0]
...
def load_gt_depths(image_list, datadir, H=None, W=None, crop_ratio=1):
    ...
    for image_name in image_list:
        ...
        if H is not None:
            ...
            depths.append(depth_resize)  # masks.append(mask_resize > 0.5)
        else:
            depths.append(depth)  # masks.append(depth > 0)
