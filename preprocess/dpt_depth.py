...
def dpt_depth(cfg, depth_save_dir):
    ...
    for data in train_loader:
        ...
        np.savez(
            os.path.join(depth_save_dir, "depth_{}.npz".format(img_name.split(".")[0])),
            pred=depth.detach().cpu(),
        )
        depth_array = depth[0].detach().cpu().numpy()
        imageio.imwrite(
            os.path.join(depth_save_dir, "{}.png".format(img_name.split(".")[0])),
            np.clip(
                255.0 / depth_array.max() * (depth_array - depth_array.min()), 0, 255
            ).astype(np.uint8),
        )
...
if __name__ == "__main__":
    ...
    depth_save_dir = os.path.join(
        cfg["dataloading"]["path"], cfg["dataloading"]["scene"][0], folder_name
    )
