...
class Renderer(nn.Module):
    ...
    def nope_nerf(
        ...
        points_world = transform_to_world(
            pixels, depth, camera_mat, world_mat, scale_mat
        )
        ...
        if normalise_ray:
            ray_vector = ray_vector / ray_vector.norm(2, 2).unsqueeze(
                -1
            )  # normalised ray vector
        else:
            d_i_gt = (
                d_i_gt / ray_vector_norm
            )  # used for guide sampling, convert dist to depth
        ...
        z_val = torch.linspace(
            0.0, 1.0, steps=full_steps - outside_steps, device=device
        )
        z_val = z_val.view(1, 1, -1).repeat(batch_size, n_points, 1)

        if sample_option == "ndc":
            z_val, pts, ray_vector_fg = self.sample_ndc(
                camera_mat, camera_world, ray_vector, z_val, depth_range=[0.0, 1.0]
            )
        elif sample_option == "uniform":
            z_val, pts, ray_vector_fg = self.sample_uniform(
                camera_world, ray_vector, z_val, add_noise, depth_range
            )
        ...
        if dist_alpha:
            ...
            dist_far = torch.empty(
                size=(batch_size * n_points, 1),
                dtype=torch.float32,
                device=dists.device,
            ).fill_(
                1e10
            ...
            alpha[:, -1] = 1.0  # enforce predicted depth>0

        weights = alpha * torch.cumprod(
            torch.cat(
                [
                    torch.ones((rgb.shape[0], 1), device=device),
                    1.0 - alpha + epsilon,
                ],
                -1,
            ),
        ...
        if not eval_ and normal_loss:
            ...
            surface_points_neig = (
                surface_points + (torch.rand_like(surface_points) - 0.5) * 0.01
            )
            pp = torch.cat([surface_points, surface_points_neig], dim=0)
            g = self.model.gradient(pp, it)
            normals_ = g[:, 0, :] / (
                g[:, 0, :].norm(2, dim=1).unsqueeze(-1) + 10 ** (-5)
            )
        ...
        if eval_ and normalise_ray:
            ...
            dist_pred = (
                dist_pred / ray_vector_norm[0]
            )  # change dist to depth, consistent with gt depth for evaluation
    ...
    def sample_ndc(
        self, camera_mat, camera_world, ray_vector, z_val, depth_range=[0.0, 1.0]
    ):
        batch_size, n_points, full_steps = z_val.shape
        focal = torch.cat([camera_mat[:, 0, 0], camera_mat[:, 1, 1]])
        ray_ori_world, ray_dir_world = get_ndc_rays_fxfy(
            focal, 1.0, rays_o=camera_world, rays_d=ray_vector
        )
        z_val = depth_range[0] * (1.0 - z_val) + depth_range[1] * z_val
        pts = ray_ori_world.unsqueeze(-2) + ray_dir_world.unsqueeze(
            -2
        ) * z_val.unsqueeze(-1)
        pts = pts.reshape(-1, 3)
    ...
    def sample_uniform(self, camera_world, ray_vector, z_val, add_noise, depth_range):
        ...
        pts = camera_world.unsqueeze(-2) + ray_vector.unsqueeze(-2) * z_val.unsqueeze(
            -1
        )
        pts = pts.reshape(-1, 3)
        ray_vector_fg = ray_vector.unsqueeze(-2).repeat(1, 1, full_steps, 1)
    ...
    def phong_renderer(self, pixels, camera_mat, world_mat, scale_mat, it):
        ...
        diffuse = (
            torch.mm(surface_normals, light).clamp_min(0).repeat(1, 3)
            * diffuse_per.unsqueeze(0).cuda()
        )
        rgb_values[network_object_mask] = (
            ambiant.unsqueeze(0).cuda() + diffuse
        ).clamp_max(1.0)
    ...
    def ray_marching(
        ...
        d_proposal = (
            torch.linspace(0, 1, steps=n_steps).view(1, 1, n_steps, 1).to(device)
        )
        d_proposal = (
            depth_range[0] * (1.0 - d_proposal)
            + d_intersect.view(1, -1, 1, 1) * d_proposal
        )

        p_proposal = (
            ray0.unsqueeze(2).repeat(1, 1, n_steps, 1)
            ...
            val = torch.cat(
                [
                    ...
                    for p_split in torch.split(
                        ...
                        int(max_points / batch_size),
                        dim=1,
                    )
                ],
                dim=1,
            ).view(batch_size, -1, n_steps)
        ...
        n = batch_size * n_pts
        d_low = d_proposal.view(n, n_steps, 1)[torch.arange(n), indices.view(n)].view(
            batch_size, n_pts
        )[mask]
        ...
        d_high = d_proposal.view(n, n_steps, 1)[torch.arange(n), indices.view(n)].view(
            batch_size, n_pts
        )[mask]
    ...
    def secant(
        ...
        for i in range(n_secant_steps):
            ...
            with torch.no_grad():
                f_mid = (
                    self.model(p_mid, batchwise=False, only_occupancy=True, it=it)[
                        ..., 0
                    ]
                    - tau
                )
    ...
    def transform_to_homogenous(self, p):
        ...
        p_homo = (
            torch.cat((p, torch.ones(batch_size, num_points, 1).to(device)), dim=2) / r
        )
        return p_homo
