...
class Loss(nn.Module):
    ...
    def get_reprojection_loss(self, rgb, rgb_refs, valid_points, rgb_refs_ori):
        ...
        for (rgb_ref, rgb_ref_ori) in zip(rgb_refs, rgb_refs_ori):
            ...
            if cfg["with_auto_mask"] == True:
                auto_mask = (
                    diff_img.mean(dim=-1, keepdim=True)
                    < (rgb - rgb_ref_ori).abs().mean(dim=-1, keepdim=True)
                ).float() * valid_points
    ...
    def get_DPT_reprojection_loss(self, rgb, rgb_refs, valid_points, rgb_img_refs_ori):
        ...
        for rgb_ref, rgb_img_ref_ori in zip(rgb_refs, rgb_img_refs_ori):
            ...
            if cfg["with_auto_mask"] == True:
                auto_mask = (
                    diff_img.mean(dim=1, keepdim=True)
                    < (rgb - rgb_img_ref_ori).abs().mean(dim=1, keepdim=True)
                ).float()
    ...
    def get_pc_loss(self, Xt, Yt):
        ...
        if match_method == "dense":
            loss1 = self.comp_point_point_error(
                Xt[0].permute(1, 0), Yt[0].permute(1, 0)
            )
            loss2 = self.comp_point_point_error(
                Yt[0].permute(1, 0), Xt[0].permute(1, 0)
            )
            loss = loss1 + loss2
        return loss
    ...
    def get_depth_consistency_loss(self, d1_proj, d2, d2_proj=None, d1=None):
        ...
        if d2_proj is not None:
            loss = 0.5 * loss + 0.5 * self.l1_loss(d2_proj, d1) / float(
                d2_proj.shape[1]
            )
        return loss
    ...
    def forward(
        ...
        **kwargs
        ...
        if (
            weights["weight_dist_2nd_loss"] != 0.0
            or weights["weight_dist_1st_loss"] != 0.0
        ):
            loss_dist_1st, loss_dist_2nd = self.get_weight_dist_loss(t_list)
        else:
            loss_dist_1st, loss_dist_2nd = (
        ...
        if weights["depth_consistency_weight"] != 0.0:
            depth_consistency_loss = self.get_depth_consistency_loss(
                d1_proj, d2, d2_proj, d1
            )
        else:
        ...
        loss = (
            weights["rgb_weight"] * rgb_full_loss
