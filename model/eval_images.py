...
from model.common import get_tensor_values, arange_pixels
...
class Eval_Images(object):
    ...
    def eval_images(
        ...
        depth_input = get_tensor_values(
            depth_input, pixels.clone(), mode="nearest", scale=True, detach=False
        )
        ...
        with torch.no_grad():
            rgb_pred = []
            depth_pred = []
            for i, (pixels_i, depth_i) in enumerate(
                zip(
                    torch.split(pixels, self.points_batch_size, dim=1),
        ...
        tqdm.write(
            "{0:4d} img: PSNR: {1:.2f}, SSIM: {2:.2f},  LPIPS {3:.2f}".format(
                img_idx, psnr, ssim, lpips_loss
            )
        )

        gt_height, gt_width = depth_gt.shape[:2]
        depth_out = cv2.resize(
            depth_out, (gt_width, gt_height), interpolation=cv2.INTER_NEAREST
        )
        ...
        depth_out = (
            np.clip(255.0 / depth_out.max() * (depth_out - depth_out.min()), 0, 255)
        ).astype(np.uint8)
        img_out = (img_out.cpu().numpy() * 255).astype(np.uint8)
        img_gt = (img_gt.cpu().numpy() * 255).astype(np.uint8)
        imageio.imwrite(
            os.path.join(img_out_dir, str(img_idx).zfill(4) + ".png"), img_out
        )
        imageio.imwrite(
            os.path.join(depth_out_dir, str(img_idx).zfill(4) + ".png"), depth_out
        )
        imageio.imwrite(
            os.path.join(img_gt_dir, str(img_idx).zfill(4) + ".png"), img_gt
        )
